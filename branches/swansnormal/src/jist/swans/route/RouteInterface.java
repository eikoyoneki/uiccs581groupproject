 ...
 * @version $Id: RouteInterface.java,v 1.1 2007/04/09 18:49:29 drchoffnes Exp $
 * @since SWANS1.0
 */
public interface RouteInterface extends NetInterface.NetHandler, JistAPI.Proxiable
{
	/**@author tariavo
	 * hook up with Network layer
	 * @param netInterface
	 */
	void setNetEntity(NetInterface netInterface);
  ...
  /**
   * Informs the network layer that a packet was dropped.
   * 
   * @param packet
   * @param packetNextHop
   */
   ...
   * NS2 Timer interface
   */
  public static interface NS2TimerInterface{
      void callback(int id, Location loc, NetAddress na, NS2Timer t, int timerId);
      /**
       * 
       * @param id id in hashmap for timer object
       * @param interval duration for timeout
       * @param timerId timer identifier for reusable timers
       */
      void timeout(Long id, long interval, int timerId);   
  }
  
  /**
   * 
   * GPSR routing entity interface.
   */
  public static interface Geo extends RouteInterface, Protocol, NS2TimerInterface {

      

      
      void sendIpMsg(Ip msg, int ni, MacAddress ma);
   ...
   */
  public static interface Dsr_NS2  extends RouteInterface, Protocol, NS2TimerInterface
  {
      void sendIpMsg(Ip msg, int ni, MacAddress ma);
      void start();
    ...
    public class BufferedPacket
    {
      ...
      public BufferedPacket(NetMessage.Ip msg)
      {
        ...
        public boolean equals(Object obj) {
            BufferedPacket bp = (BufferedPacket)obj;
            return this.msg.getPayload().equals(bp.msg.getPayload());
        }
    }
